...
* **`manage_chart_viewer` (agent-pca)**: Controls the native TC2000-style desktop chart viewer running on the user's screen.
  * Actions: `DISPLAY_STOCK` (loads historical candles, indicators, and topbar metrics for a `ticker`, optional `preset`: `'default'` [SMA 50/200 + BB 20], `'trend_template'` [6 Minervini SMAs + Topbar RS/Minervini/ADR], `'momentum'` [EMA 8/21], `'clean'` [candles only] OR any dynamically created user preset like `'qmaggi'`), `OPEN_WINDOW` (opens custom window), `ADD_ANNOTATION` (draws support/resistance `hline`, trendlines, boxes, or buy/sell `trade_marker`), `REMOVE_ANNOTATION` (removes drawing object), `SET_TOPBAR` (displays metrics in topbar grid), `STATUS` (checks viewer connection & open windows), `CLOSE_WINDOW` (closes window), `SCREENSHOT` (captures screenshots of all open windows in standard 640x480 or hires 800x600 [optional `hires: true` or `resolution: "800x600"`], with edge-sharpening to eliminate blur, saves them in `/dsh_playground`, and returns capture ID and filepaths for visual UI debugging).


* **`manage_chart_presets` (agent-pca)**: Manage dynamically created chart presets (CRUD) and their overlay indicators (like SMA/EMA).
  * Actions: `CREATE`, `GET`, `LIST`, `UPDATE`, `DELETE`. Specify `preset_id` and the `members` (features like `sma_10`, `ema_20`, `bb_20`, `adr_1_pct`) to automatically render them.
